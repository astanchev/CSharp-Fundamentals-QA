        ...
        private static void DistributeStudentsIntoGroups()
        {
            foreach (var town in towns)
            {
                List<Student> townStudents = town.Students
                                                        ...
                                                        .ThenBy(s => s.Email)
                                                        .ToList();
                while (townStudents.Count > 0)
                {
                    Group group = new Group();
                    group.Town = town;
                    group.Students = townStudents.Take(group.Town.SeatsCount).ToList();
                    townStudents = townStudents.Skip(group.Town.SeatsCount).ToList();
                    groups.Add(group);
                }
        ...
        private static void ReadTownsAndStudents()
        {
            while (true)
            {
                ...
                if (input.Contains("=>"))
                {
                    ...
                    string[] inputline = input.Trim().Split(new string[] {" => "}, StringSplitOptions.RemoveEmptyEntries);
                ...
                else
                {
                    ...
                    currentStudent.RegistrationDate = DateTime.ParseExact(inputline[3], "d-MMM-yyyy", CultureInfo.InvariantCulture);
